resource "azurerm_virtual_machine_scale_set" "user16vmss2" {
name = "user16vmss2"
...
#ssh -i ~/.ssh/id_rsa myadmin@104.40.10.17 -p 50001  (첫번째 서버 접속)
#ssh -i ~/.ssh/id_rsa myadmin@104.40.10.17 -p 50003  (두번째 서버 접속)
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user16-subnet2.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user16-bpepool2.id]
